    ...
    def generate_rules(self):
        if self:
            if len(self.children) == 2:
                self.children[0].generate_rules()
                child1 = self.children[0].label+"^"+self.children[0].parent.label
                child2 = self.children[1].label+"^"+self.children[1].parent.label
                if self.parent is not None:
                    LHS = self.label + "^" + self.parent.label
                else:
                    LHS = self.label 
                text = LHS + "->" + child1 + " "  + child2
                count[LHS] += 1
                trace[text] += 1

                text1 = self.label + "->" + self.children[0].label + " "  + self.children[1].label
                if self.label != LHS:
                    count[self.label] += 1
                trace[text1] += 1
                

                # text = self.label + "->" + self.children[0].label + " "  + self.children[1].label
                self.children[1].generate_rules()
            elif len(self.children) == 1:
                self.children[0].generate_rules()
                text1 = self.label + "->" + self.children[0].label
                count[self.label] +=1
                trace[text1] += 1


                LHS = self.label + "^" + self.parent.label
                text = LHS + "->" + self.children[0].label
                count[LHS] +=1
                trace[text] += 1
            else:
                return
...
class Tree(object):
    ...
    def generate_rules(self):
        return self.root.generate_rules()
